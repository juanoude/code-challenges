...
var _ = Describe("MonsterController", func() {
	...
	Describe("Import CSV", func() {
		...
		BeforeEach(func() {
			writer, csvContent = utilstests.GenerateMultipartFormFile("data/monsters-correct.csv")
		})

		JustBeforeEach(func() {
			req, _ := http.NewRequest(http.MethodPost, "/monsters/import", &csvContent)
		...
		Context("should fail when importing csv file with an empty monster", func() {
			BeforeEach(func() {
				writer, csvContent = utilstests.GenerateMultipartFormFile("data/monsters-empty-monster.csv")
			})
		...
		Context("should fail when importing csv file with wrong or inexistent columns.", func() {
			BeforeEach(func() {
				writer, csvContent = utilstests.GenerateMultipartFormFile("data/monsters-wrong-column.csv")
			})
		...
		Context("should import all the CSV objects into the database successfully", func() {
			It("status code should be 202", func() {
