...
func FetchAllMonsters(context *gin.Context) {
...
func ImportCSV(context *gin.Context) {
	...
	if err = validateMonsterColumns(records[0]); err != nil {
...
func validateMonsterColumns(records []string) error {
	for index, record := range records {
		fmt.Println(index, record, columnNames[index])
		if err := validateColumn(record, columnNames[index]); err != nil {
			return err
		}
	}
...
func validateColumn(recordField string, fieldName string) error {
	if recordField != fieldName {
